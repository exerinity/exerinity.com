...
// Completely unrelated to stars, but this is for domains: find site-brand, find examples of "exerinity.com" (the old domain), and make it window.location.hostname
document.querySelectorAll(".site-brand").forEach(el => {
    el.innerHTML = el.innerHTML.replace(/exerinity.com/g, window.location.hostname);

    // and for the title
    document.title = document.title.replace(/exerinity.com/g, window.location.hostname);
});
